...
| Нельзя перетащить PDF из Проводника в окно | В 0.1.2 для окна выставлено `dragDropEnabled: false` — иначе Tauri перехватывает drop и ломает HTML5 DnD редактора. Пересоберите и переустановите. |
