...
| Ярлык / иконка приложения «старая», хотя версия новая | Windows кэширует иконки по пути. Нужна сборка **≥ 0.1.1** (`npm run tauri:build`), затем переустановка. Если ярлык всё ещё старый: удалить ярлык с рабочего стола, в Проводнике открыть папку установки → ПКМ по `PDF Manager.exe` → «Отправить» → «Рабочий стол»; либо перезапустить проводник (`taskkill /f /im explorer.exe` → `start explorer`) / очистить кэш иконок (`ie4uinit.exe -show`). |
